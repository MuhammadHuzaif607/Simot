...
import { clearRepairState, fetchRepairProduct, updateRepairDetails } from "../redux/slices/repairSlice";
...
const DeviceModal = ({ open, onClose, selectedDevice }) => {
  ...
  const {repairProduct,
      ...
      loading} = useSelector((state)=>state.repair)
  ...
  const handleSubmit = async() => {
    try {
      ...
      if (selectedOrdinary && technicianCost[selectedOrdinary]) {
        ordinaryTechnician = { [selectedOrdinary]: technicianCost[selectedOrdinary] };
      }
      ...
      const res = await dispatch(updateRepairDetails({imei:selectedImei, repairData:ObjtoSend}));
              ...
              <div key={option} className="flex items-center mb-2">
                ...
                <label
                  ...
                  className={`cursor-pointer flex items-center w-full p-3 rounded-md text-white transition ${
                    selectedOrdinary === option ? "bg-blue-500" : "bg-gray-700 hover:bg-gray-600"
                  }`}
                >
                  <div className={`w-5 h-5 border-2 rounded-md flex justify-center items-center ${
                    selectedOrdinary === option ? "border-blue-300 bg-blue-600" : "border-gray-400"
                  }`}>
                    {selectedOrdinary === option && <div className="w-3 h-3 bg-white rounded-md"></div>}
                  </div>
            ...
            {extraOptions.map((option) => (
              <div key={option} className="flex items-center mb-2">
                ...
                <label
                  htmlFor={option}
                  className={`cursor-pointer flex items-center w-full p-3 rounded-md text-white transition ${
                    selectedExtra.includes(option) ? "bg-green-500" : "bg-gray-700 hover:bg-gray-600"
                  }`}
                >
                  <div className={`w-5 h-5 border-2 rounded-md flex justify-center items-center ${
                    selectedExtra.includes(option) ? "border-green-300 bg-green-600" : "border-gray-400"
                  }`}>
                    {selectedExtra.includes(option) && <div className="w-3 h-3 bg-white rounded-md"></div>}
                  </div>
                  <span className="ml-3">{formatOptionDisplay(option)}</span>
                </label>
              </div>
