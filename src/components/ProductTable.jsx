...
const ProductTable = ({
  ...
  const statusOptions = ["To Be Repaired", "Ready for Sale", "In Progress", "In Confirmation"];
  ...
  const sortedProducts = [...products]
    .filter((product) => {
      const conditionMatch =
        !searchFilters.condition || product.condition === searchFilters.condition;
      const brandMatch =
        !searchFilters.deviceBrand || product.deviceBrand === searchFilters.deviceBrand;
      const modelMatch =
        !searchFilters.deviceModel || product.deviceModel === searchFilters.deviceModel;
      const colorMatch =
        !searchFilters.deviceColor || product.deviceColor === searchFilters.deviceColor;
      const gradeMatch =
      ...
      const priceMatch =
        (!searchFilters.priceFrom || product.price >= parseFloat(searchFilters.priceFrom)) &&
        (!searchFilters.priceTo || product.price <= parseFloat(searchFilters.priceTo));
      ...
      <div className="flex flex-col items-center justify-between mb-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <select
            ...
            onChange={(e) => handleDropdownChange("deviceBrand", e.target.value)}
          >
          ...
          <select
            ...
            onChange={(e) => handleDropdownChange("deviceModel", e.target.value)}
          >
          ...
          <select
            ...
            onChange={(e) => handleDropdownChange("deviceColor", e.target.value)}
          >
          ...
          <div className="flex space-x-2">
            <input
              ...
              onChange={(e) => handleDropdownChange("priceFrom", e.target.value)}
            />
            <input
              type="number"
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-center text-white text-sm">
            <thead className="bg-[#393b63] sticky top-0 text-sm">
              <tr>
                {shouldShowColumn("deviceType") && <th className="py-2 px-2">Device Type</th>}
                {shouldShowColumn("condition") && <th className="py-2">Condition</th>}
                {shouldShowColumn("price") && <th className="py-2">Price</th>}
                {shouldShowColumn("grade") && <th className="py-2">Grade</th>}
                {shouldShowColumn("deviceBrand") && <th className="py-2">Brand</th>}
                {shouldShowColumn("deviceModel") && <th className="py-2">Model</th>}
                {shouldShowColumn("deviceColor") && <th className="py-2">Color</th>}
                {shouldShowColumn("memory") && <th className="py-2">Memory GB</th>}
            ...
            <tbody>
              <tr>
                <td colSpan="13" className="py-4">No products found</td>
              </tr>
            </tbody>
          </table>
        </div>
