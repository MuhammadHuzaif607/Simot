  ...
  useEffect(() => {
    if (repairProduct) {
      const filtered = repairProduct.filter(product => 
        searchImei ? product.imei.toLowerCase().includes(searchImei.toLowerCase()) : true
      );
        ...
        <table className="w-full border border-gray-300 rounded-md">
          ...
          <tbody>
            {filteredProducts.length === 0 ? (
              <tr>
                <td colSpan="12" className="text-center py-4 text-white">No products found</td>
              </tr>
            ) : (
              filteredProducts.map((product) => (
                ...
                >
                  ...
                  <td className="px-4 py-2">{product?.repairInfo?.reRepairInfo || "No ReRepair"}</td>
                </tr>
              ))
            )}
          </tbody>
